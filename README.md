...
**Declare `services.ansible = { … };`, get one `ansible.service` systemd unit that runs a composed playbook against localhost, and fail loud on activation if it crashes.**

## Quickstart

```nix
{
  inputs.system-manager-ansible.url = "github:Dauliac/system-manager-ansible";

  outputs = { self, nixpkgs, system-manager, system-manager-ansible, ... }: {
    systemConfigs.myhost = system-manager.lib.makeSystemConfig {
      modules = [
        system-manager-ansible.systemManagerModules.default
        ({ pkgs, ... }: {
          services.ansible = {
            enable  = true;
...
- **Two module APIs**:
  - `nixosModules.default` / `systemManagerModules.default` — system-wide, `become: yes`, `systemd.services.ansible`.
  - `homeManagerModules.default` — user-scoped, `become: no`, `systemd.user.services.ansible`.
