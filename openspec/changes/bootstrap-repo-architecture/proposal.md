...
`system-manager-ansible` extracts and generalizes the ad-hoc `bootstrap.yml`
...
- **One `services.ansible` module** per host, matching the shape of `services.nginx`, `services.postgresql`, `services.k3s` — `enable`, `package`, and a small handful of typed options. No invented terminology; nothing that reads unlike a stock NixOS module.
- **One module invocation → one playbook → one systemd unit** (`ansible.service`). Bootstrap-scale workloads (tens of tasks) run comfortably in a single ansible invocation. Task-level batching (`apt: name: [a,b,c]`) provides the parallelism that matters. Multiple systemd units would be strictly worse: harder to reason about, harder to fail-loud, and offers no meaningful concurrency on a single localhost.
- **One unified role namespace** — `services.ansible.roles.<name>` accepts either a disk-backed role (`roles/<name>/` on disk, typed schema in `meta/nix-options.nix`) or an inline role authored directly in Nix (via `.tasks` / `.handlers` fields), or a hybrid of both. The composer resolves which at build time.
- **Definition and call in the same block.** Any nix file anywhere in the module system can write `services.ansible.roles.<name> = { … };` and its tasks, vars and metadata merge in. Adding a feature = adding a nix file; removing a feature = deleting it. No central manifest.
...
- **Two module APIs, one core.** `nixosModules.default` and `systemManagerModules.default` are the *same* module (system context, `systemd.services.ansible`, `become: yes`). `homeManagerModules.default` mirrors the option tree exactly but wires `systemd.user.services.ansible` and drops `become`. A role's context is decided by which module imports it, not by anything the role declares.
...
- Introduce a **NixOS / system-manager module** at `services.ansible` exposing:
...
- Introduce a **home-manager module** mirroring the exact same option tree, but generating `systemd.user.services.ansible` and passing `become = false` to composition.
...
- `nix-module-api`: the outward-facing `services.ansible` option tree exposed by both `nixosModules.default` / `systemManagerModules.default` (identical) and `homeManagerModules.default` (user-context mirror), including the post-deploy hook that fails activation when the generated unit crashed.
...
- **New repo surface.** Consumers of `dotfiles` currently importing `inputs.self.modules.systemManager.debian` will migrate to `inputs.system-manager-ansible.systemManagerModules.default` and declare `services.ansible = { enable = true; roles = { … }; };`. The old inline module can be deleted once migration lands.
...
- **Ansible toolchain is caller-configurable.** `services.ansible.package` defaults to `pkgs.ansible` but can be overridden per host.
